...
const QR_PEOPLE_STAGES = {
  paseo: ["wss://paseo-people-next-rpc.polkadot.io"],
...
// The QR signer derives the account from the paired wallet, so a local-keystore option is
// contradictory; reject it rather than silently ignoring it.
export function assertSignerOptions(
  ...
  const conflicting = (Object.keys(KEYSTORE_AUTH_FLAGS) as KeystoreAuthOption[])
    .filter((key) => options[key] !== undefined)
    .map((key) => KEYSTORE_AUTH_FLAGS[key]);
  if (conflicting.length > 0) {
    throw new Error(
      `--signer qr signs with the paired mobile wallet, so it cannot be combined with ` +
        `${conflicting.join(", ")}. Remove ${conflicting.length > 1 ? "those flags" : "that flag"}.`,
    );
  }
}
